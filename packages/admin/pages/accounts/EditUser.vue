  ...
  <div class="wrap-edit mt-3">
    ...
    <b-form class="mt-2">
      ...
      <b-form-group id="input-role" label="Chức vụ:" label-for="input-role">
        <b-form-select id="input-role" @input="updateRold" v-model="form.role" :options="roleOptions" :disabled="boolean"></b-form-select>
      </b-form-group>
...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      await this.$http.$put(`users/${this.selected.role.name}`, {
        ...submit,
      })
      this.$router.go()
    },
